        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Const_value_test(1.f, 1.f, 1.f);//常值检查用

                    float area = CalcArea(x/float(width),y/float(height),width,height);
                    for (int l = 0;l<=SHOrder;l++){
                        for (int m = -l;m<=l;m++){
                            float SH_value = sh::EvalSH(l,m, Eigen::Vector3d(dir.x(), dir.y(), dir.z()).normalized());
                            SHCoeffiecents[sumWeight++] += area * Le * SH_value;
                        }
                    }
                    sumWeight = 0;
                }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        m_TransportSHCoeffs.resize(SHCoeffLength, mesh->getVertexCount()*2);
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                float BSDF = 1.f / 3.14159f;//didduse
                ...
                else
                {
                    ...
                    float cos = wi.dot(n);
                    return cos>0?(scene->rayIntersect(Ray3f(v+ wi.normalized()*0.001, wi.normalized()))?0: cos):0;
                }
            };
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            int Interreflection_SampleCount = 16;
            float BSDF = 1.f / 3.14159f;//diffuse
            std::random_device rd;
            std::mt19937 gen(rd());
            std::uniform_real_distribution<> rng(0.0, 1.0);
            //m_TransportSHCoeffs.resize(SHCoeffLength, mesh->getVertexCount()*2);
            for (int i=0; i < mesh->getVertexCount(); i++) {
                for (int t=0; t < Interreflection_SampleCount; t++) {
                    for (int p=0; p < Interreflection_SampleCount; p++) {
                        Intersection its;
                        float a = (t+ rng(gen)) / float(Interreflection_SampleCount);
                        float b = (p+ rng(gen)) / float(Interreflection_SampleCount);
                        float phi = 2.f * 3.14159f * a;
                        float zeta = acos(b*2.f-1.f);
                        auto dir = sh::ToVector(phi, zeta).normalized();
                        auto vdir = Vector3f(dir.x(), dir.y(), dir.z()).normalized();
                        float H = vdir.dot(mesh->getVertexNormals().col(i).normalized());
                        if (H<=0) {
                            continue;
                        }
                        Vector3f o = mesh->getVertexPositions().col(i);
                        if (scene->rayIntersect(Ray3f(o+ vdir*0.001, vdir), its)) {//命中三角形
                            auto SH0 = m_TransportSHCoeffs.col(its.tri_index.x());
                            auto SH1 = m_TransportSHCoeffs.col(its.tri_index.y());
                            auto SH2 = m_TransportSHCoeffs.col(its.tri_index.z());
                            auto color = SH0 * its.bary.x()+SH1 * its.bary.y()+SH2 * its.bary.z();
                            auto pos2inter = its.p - o;
                            Vector3f p2i = Vector3f(pos2inter.x(), pos2inter.y(), pos2inter.z());
                            float distense_pow = std::max(p2i.dot(p2i), 0.0001f);
                            for (int j = 0; j < SHCoeffLength; j++) {
                                m_TransportSHCoeffs.col(mesh->getVertexCount()+i).coeffRef(j) += color[j]*H;
                            }
                        }
                    }
                }
            }
            float inv_ = 1. / Interreflection_SampleCount;
            inv_ = pow(inv_, 2.0);
            for (int i = 0; i < mesh->getVertexCount(); i++) {
                for (int j = 0; j < SHCoeffLength; j++) {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += m_TransportSHCoeffs.col(mesh->getVertexCount() + i).coeffRef(j) * inv_;
                }
            }
        }
